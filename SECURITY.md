...
- Anything that lets a scan or a cleanup operation touch a file outside
...
- Denial of service by an authorised client. The agent trusts the shell it was
  ...
  Even so, machine-wide changes (Defender's protections, audit policy, firewall rules,
  the protection switch, and clearing a cache outside the caller's own profile)
  also need the caller's own token to be elevated, checked in the server before
  any handler runs. The window never runs elevated; it asks through Windows'
  ordinary permission prompt, and an approved short-lived copy makes that one
  change and exits.
